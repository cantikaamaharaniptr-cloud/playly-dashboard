...
(function () {
  ...
  async function uploadViaR2(id, blob) {
    try {
      ...
      if (!signResp.ok || !signData || !signData.ok) {
        ...
        // daily_quota_exceeded — kuota harian habis. Hard error, jangan
        // fallback ke Supabase (Supabase pun ga akan punya kuota terpisah).
        if (signData && signData.error === "daily_quota_exceeded") {
          const tier = signData.tier || "free";
          const quotaGB = Math.round((signData.quotaBytes || 0) / (1024 * 1024 * 1024));
          const usedMB = Math.round((signData.usedBytes || 0) / (1024 * 1024));
          return {
            ok: false,
            error: "daily_quota_exceeded",
            tier,
            message: `Kuota upload harian habis (${usedMB} MB / ${quotaGB} GB). ${tier === "premium" ? "Reset 00:00 UTC." : "Upgrade Premium untuk 10 GB/hari atau tunggu reset 00:00 UTC."}`,
          };
        }
  ...
  function injectMainScript() {
    ...
    s.src = "script.js?v=20260529-ve-tabs-export-v635";
